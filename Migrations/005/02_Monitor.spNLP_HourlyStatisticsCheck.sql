...
CREATE or ALTER  PROCEDURE [Monitor].[spNLP_HourlyStatisticsCheck] 
AS
...
DECLARE
	@Subject				varchar(200)	= 'Hourly NLP Statistics Check ' + CAST(GetDate() AS varchar(30)),
	@Body					varchar(max)	= '';


CREATE TABLE #NlpHourlyStatistics
( TypeOfStatistic Varchar(250) null,
  ValueOfStatistic Varchar(250) null,
  ExecutionTime Varchar(250) null);

Insert into #NlpHourlyStatistics
	  ( TypeOfStatistic  ,
		ValueOfStatistic  ,
		ExecutionTime  )
...
Insert into #NlpHourlyStatistics
exec [Monitor].[spNLP_NumberOfNotesFinishedPerMinuteLastHour]  
...
SELECT
    @Body = CAST((
        ...
        FROM (
            ...
            FROM #NlpHourlyStatistics
			) AS DataColumns
        FOR XML PATH(''), TYPE
    ).value('.', 'NVARCHAR(MAX)') AS NVARCHAR(MAX) );

drop table #NlpHourlyStatistics;
